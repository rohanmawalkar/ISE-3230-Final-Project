...
# Population: civilians only (workers + potential soldiers)
civ = m.addVars(states, lb=0.0, vtype=GRB.CONTINUOUS, name="civ")

# people working
popNew = m.addVars(turns, lb=0, vtype=GRB.INTEGER, name="popNew")

# Soldiers owned
sol1_start = m.addVars(turns, lb=0, vtype=GRB.INTEGER, name="sol1_start")
sol2_start = m.addVars(turns, lb=0, vtype=GRB.INTEGER, name="sol2_start")

# Soldiers 
sol1_alive = m.addVars(states, lb=0.0, vtype=GRB.CONTINUOUS, name="sol1_alive")
sol2_alive = m.addVars(states, lb=0.0, vtype=GRB.CONTINUOUS, name="sol2_alive")

# Workers assigned
spop = m.addVars(turns, lb=0.0, vtype=GRB.CONTINUOUS, name="spop")
fpop = m.addVars(turns, lb=0.0, vtype=GRB.CONTINUOUS, name="fpop")
wpop = m.addVars(turns, lb=0.0, vtype=GRB.CONTINUOUS, name="wpop")
gpop = m.addVars(turns, lb=0.0, vtype=GRB.CONTINUOUS, name="gpop")
...
# Starting population (all civilians, no soldiers)
...
# Population number
for t in turns:
    # invasion losses after this turn (if any)
    loss_expr = 0.0
    if t in turn_to_inv:
        k = turn_to_inv[t]
        loss_expr = pl[k]
